-- FightIQ V5.9 — correction des champs uniques pendant la fusion canonique.
--
...
-- Cette version libère temporairement les identifiants Cito et slugs uniques
-- de la fiche source avant de les transférer fill-null-only vers la cible.
--
...
begin
    ...
    loop
        ...
        -- Libère les valeurs uniques de la fiche source avant de les rattacher
        -- à la cible. source_row les conserve en mémoire. En cas d'erreur
        -- ultérieure, la transaction restaure automatiquement la fiche source.
        ...
        -- Les valeurs uniques ont été libérées sur la source. Elles peuvent
        -- maintenant être transférées sans collision, toujours sans écraser
        -- une valeur déjà présente sur la fiche canonique.
        update public.fighters
        set cito_id = coalesce(cito_id, source_primary_cito),
            cito_slug = coalesce(cito_slug, source_row.cito_slug),
            slug = coalesce(slug, source_row.slug),
